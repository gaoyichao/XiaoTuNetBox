...
namespace net {
    ...
    static const std::string _base64_ = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"; //=
    std::string Base64Encode(const std::string &in) {
        ...
        for (uint8_t c : in) {
            ...
            valb += 8;
            while (valb >= 0) {
                out.push_back(_base64_[(val >> valb) & 0x3F]);
                valb -= 6;
            }
        }

        if (valb>-6)
            out.push_back(_base64_[((val << 8) >> (valb + 8)) & 0x3F]);
