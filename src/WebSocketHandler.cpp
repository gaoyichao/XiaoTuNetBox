...
namespace net {
    ...
    uint8_t const * WebSocketHandler::GetData(int n, uint8_t const * & begin, uint8_t const * & end)
    {
        int nbuf = end - begin;
    ...
    uint8_t const * WebSocketHandler::OnOpen(uint8_t const * begin, uint8_t const * end)
    {
    ...
    //! @return 若切换状态，则返回消费数据之后的起始地址，
    //!         若消费完所有 n 个字节，仍然没有切换状态，则返回 nullptr 
    uint8_t const * WebSocketHandler::OnReadingLen16(uint8_t const * begin, uint8_t const * end)
    {
        uint8_t const * crlf = GetData(2, begin, end);
        if (nullptr == crlf)
            return nullptr;
    ...
    uint8_t const * WebSocketHandler::OnReadingLen64(uint8_t const * begin, uint8_t const * end)
    {
    ...
    //! @return 若切换状态，则返回消费数据之后的起始地址，
    //!         若消费完所有 n 个字节，仍然没有切换状态，则返回 nullptr
    uint8_t const * WebSocketHandler::OnReadingMask(uint8_t const * begin, uint8_t const * end)
    {
    ...
    //! @return 若切换状态，则返回消费数据之后的起始地址，
    //!         若消费完所有 n 个字节，仍然没有切换状态，则返回 nullptr
    uint8_t const * WebSocketHandler::OnReadingPayload (uint8_t const * begin, uint8_t const * end)
    {
    ...
    //! @return 若切换状态，则返回消费数据之后的起始地址，
    //!         若消费完所有 n 个字节，仍然没有切换状态，则返回 nullptr 
    uint8_t const * WebSocketHandler::HandleMsg(uint8_t const * begin, uint8_t const * end)
    {
        while (begin < end) {
            if (eOpen == mState) {
